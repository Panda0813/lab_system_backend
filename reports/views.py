...
def get_usage_rate(request):
    try:
        obj = EquipmentBorrowRecord.objects.filter(is_borrow=True, is_return=True)
...
def get_equipment_fee(request):
    try:
        obj = EquipmentBorrowRecord.objects.filter(~Q(total_amount=None), is_borrow=True, is_return=True)
        project_id = request.GET.get('project')
        if project_id:
            obj = obj.filter(project_id=project_id)
